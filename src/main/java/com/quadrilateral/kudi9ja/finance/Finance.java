...
public final class Finance {
    ...
    /**
     * The ceiling a customer may borrow to, before the hard maximum caps it.
     *
     * <pre>
     *   offer = loanBaseCap
     *         + totalSaved x loanSavingsMultiple
     *         + (creditScore - loanScoreBaseline) x loanScorePerPoint
     *   offer = min(offer, headroom, maxLoanAmount)
     *   offer = floor(offer / loanOfferRounding) x loanOfferRounding
     *   if offer &lt; minLoanAmount then not eligible
     * </pre>
     *
     * @return the offer, or zero when the customer is not eligible
     */
    public static BigDecimal loanOffer(
            PlatformSettings s, BigDecimal totalSaved, int creditScore, BigDecimal headroom) {
        if (Money.isZeroOrLess(headroom)) {
            return Money.zero();
        }

        BigDecimal offer = Money.of(s.getLoanBaseCap());
        offer = Money.add(offer, Money.multiply(totalSaved, s.getLoanSavingsMultiple()));

        // A score below the baseline subtracts, which is the point: a thin file
        // is offered less than a customer who has built one.
        BigDecimal scorePart = BigDecimal.valueOf(creditScore - s.getLoanScoreBaseline())
                .multiply(s.getLoanScorePerPoint());
        offer = Money.add(offer, scorePart);

        offer = Money.min(offer, headroom);
        offer = Money.min(offer, s.getMaxLoanAmount());
        offer = Money.floorAtZero(offer);
        offer = Money.floorTo(offer, s.getLoanOfferRounding());

        return Money.lt(offer, s.getMinLoanAmount()) ? Money.zero() : offer;
    }

    /**
     * How much more this customer may borrow right now: the hard maximum less
     * the principal of every open loan.
     */
    public static BigDecimal headroom(PlatformSettings s, BigDecimal openLoanPrincipal) {
        return Money.floorAtZero(Money.subtract(s.getMaxLoanAmount(), openLoanPrincipal));
    }

    // ── Credit score ───────────────────────────────────────────────────────

    /**
     * A score out of 850, built only from what the customer has done with
     * Kudi9ja.
     *
     * <p>This is <b>Kudi9ja's own view</b>, not a credit-bureau score. The
     * Privacy Policy says so, and says the customer may demand a human review
     * of any automated decision that goes against them.
     */
    public static int creditScore(
        ...
        int score = s.getCreditBaseScore();
        score += clamp(plansOpened * s.getCreditPointsPerPlan(), 0, s.getCreditPlanPointsCap());

        int savingsPoints = Money.calc(totalSaved)
                .divide(s.getCreditNairaPerSavingsPoint(), 0, RoundingMode.FLOOR)
                .intValue();
        score += clamp(savingsPoints, 0, s.getCreditSavingsPointsCap());

        score += clamp(loansRepaid * s.getCreditPointsPerRepaidLoan(), 0, s.getCreditRepaidPointsCap());

        if (anyOverdue) {
            score -= s.getCreditOverduePenalty();
        }
        if (fullyVerified) {
            score += s.getCreditVerifiedBonus();
        }
        return clamp(score, s.getCreditScoreFloor(), s.getCreditScoreCeiling());
    }

    /** The band the app prints beside the number. */
    public static String creditBand(int score) {
        if (score >= 750) {
            return "Excellent";
        }
        if (score >= 680) {
