...
class FinanceTest {
    ...
    @Nested
    @DisplayName("The credit score is built only from what a customer has done here")
    class CreditScore {

        /**
         * <b>Nothing is given away.</b> There is no sign-up bonus and no free
         * credit: a brand-new account sits at the base score and nowhere above
         * it.
         */
        @Test
        @DisplayName("a brand-new account starts at the base score")
        void newAccountStartsAtBase() {
            int score = Finance.creditScore(
                    settings, 0, BigDecimal.ZERO, 0, false, false);

            assertThat(score).isEqualTo(settings.getCreditBaseScore());
        }

        @Test
        @DisplayName("the score is clamped to its floor and ceiling")
        void staysWithinBounds() {
            int best = Finance.creditScore(
                    settings, 100, new BigDecimal("100000000"), 100, false, true);
            int worst = Finance.creditScore(
                    settings, 0, BigDecimal.ZERO, 0, true, false);

            assertThat(best).isBetween(settings.getCreditScoreFloor(), settings.getCreditScoreCeiling());
            assertThat(worst).isBetween(settings.getCreditScoreFloor(), settings.getCreditScoreCeiling());
        }

        @Test
        @DisplayName("an overdue loan costs points, and clearing it gives them back")
        void overduePenaltyApplies() {
            int clean = Finance.creditScore(settings, 3, new BigDecimal("500000"), 2, false, true);
            int overdue = Finance.creditScore(settings, 3, new BigDecimal("500000"), 2, true, true);

            assertThat(clean - overdue).isEqualTo(settings.getCreditOverduePenalty());
        }

        @Test
        @DisplayName("each contribution is capped, so no single factor can carry the score")
        void factorsAreCapped() {
            int manyPlans = Finance.creditScore(
                    settings, 1000, BigDecimal.ZERO, 0, false, false);

            assertThat(manyPlans)
                    .isEqualTo(settings.getCreditBaseScore() + settings.getCreditPlanPointsCap());
        }
    }
    ...
    @Nested
    @DisplayName("The loan offer is bounded by the headroom and the hard maximum")
    class LoanOffer {

        @Test
        @DisplayName("an offer never exceeds the maximum loan amount")
        void neverExceedsTheMaximum() {
            BigDecimal offer = Finance.loanOffer(
                    settings,
                    new BigDecimal("500000000"),
                    settings.getCreditScoreCeiling(),
                    Finance.headroom(settings, BigDecimal.ZERO));

            assertThat(offer).isLessThanOrEqualTo(settings.getMaxLoanAmount());
        }

        @Test
        @DisplayName("open loans eat into the headroom")
        void openLoansReduceHeadroom() {
            BigDecimal headroom = Finance.headroom(settings, new BigDecimal("4000000"));

            assertThat(headroom).isEqualByComparingTo("1000000.00");
        }

        @Test
        @DisplayName("an offer is rounded down, never up")
        void roundsDown() {
            BigDecimal offer = Finance.loanOffer(
                    settings,
            ...
            assertThat(offer.remainder(settings.getLoanOfferRounding()))
                    .isEqualByComparingTo(BigDecimal.ZERO);
        }

        /**
         * What a brand-new account is actually offered, pinned so it cannot
         * drift unnoticed.
         *
         * <p>An account with nothing saved and no history still scores 560,
         * because that is the credit base score — and the offer formula pays
         * ₦400 for every point above the <b>loan</b> score baseline of 500,
         * which is a different number. So a fresh account is offered
         * 100,000 + 0 + 60 × 400 = ₦124,000, floored to ₦120,000.
         *
         * <p>That is the published formula working exactly as written, and it
         * is credit rather than a gift — it has to be repaid, so it does not
         * cross the rule that nothing is given away. It is worth pinning all
         * the same: the two baselines are separately settable, and moving
         * either one moves what an unverified stranger is offered on the day
         * they sign up.
         */
        @Test
        @DisplayName("a brand-new account is offered ₦120,000, from the published formula")
        void freshAccountOfferIsPinned() {
            BigDecimal offer = Finance.loanOffer(
                    settings,
                    BigDecimal.ZERO,
                    settings.getCreditBaseScore(),
                    Finance.headroom(settings, BigDecimal.ZERO));

            assertThat(offer).isEqualByComparingTo("120000.00");
        }

        /**
         * A thin file is offered less, not more. A score below the loan
         * baseline subtracts, which is the point of having a baseline.
         */
        @ParameterizedTest(name = "a score of {0}, below the baseline, is offered less than the base cap")
        @ValueSource(ints = {300, 400, 499})
        void aScoreBelowTheBaselineSubtracts(int score) {
            BigDecimal offer = Finance.loanOffer(
                    settings, BigDecimal.ZERO, score, Finance.headroom(settings, BigDecimal.ZERO));

            assertThat(offer).isLessThan(settings.getLoanBaseCap());
        }
    }
}
